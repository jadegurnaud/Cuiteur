...
//Récupération de l'id de l'utilisateur dont on veut les abonnés
...
gk_aff_debut('Cuiteur | Abonnés', '../styles/cuiteur.css');
if($id==$_SESSION['usID']){
    gk_aff_entete(true, "Vos abonnés");

}else{
    gk_aff_entete(true, "Les abonnés de ".gk_html_proteger_sortie($t2['usPseudo']));
}
gk_aff_infos();


//Affichage du résumé de l'utilisateur
echo '<form action="abonnes.php" method="GET">';
gk_utilisateur_infos($id);

//Affichage de la liste d'utilisateurs qui sont abonné à l'utilisateur
...
if (mysqli_num_rows($res) == 0){
    echo '<li>Votre fil d\'abonnes est vide</li>';
}
else{
...
echo '</ul>';
echo '<input type="submit" name="btnValiderAbonne" value="Valider">',
'</form>';
